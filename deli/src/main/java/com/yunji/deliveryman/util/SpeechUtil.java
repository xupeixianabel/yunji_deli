...
//        Log.d("tag54", "初始化失败,错ss 误码：");
    ...
    private SynthesizerListener mTtsListener = new SynthesizerListener() {
        ...
        public void onSpeakBegin() {
            Log.i(YunjiConfig.log_tag, "开始播放");
        }

        @Override
        public void onSpeakPaused() {
            Log.i(YunjiConfig.log_tag, "暂停播放");
        }

        @Override
        public void onSpeakResumed() {
            Log.i(YunjiConfig.log_tag, "继续播放");
        }

        @Override
        public void onBufferProgress(int percent, int beginPos, int endPos, String info) {
            // TODO 缓冲的进度
            Log.i(YunjiConfig.log_tag, "缓冲 : " + percent);
        }

        @Override
        public void onSpeakProgress(int percent, int beginPos, int endPos) {
            // TODO 说话的进度
            Log.i(YunjiConfig.log_tag, "合成 : " + percent);
        }

        @Override
        public void onCompleted(SpeechError error) {
            if (error == null) {
                Log.i(YunjiConfig.log_tag, "播放完成");

            } else if (error != null) {
                Log.i(YunjiConfig.log_tag, error.getPlainDescription(true));
            }
